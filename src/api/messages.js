...
app.post('/messages', async (req, res) => {
  ...
  if (validated.exists) {
    ...
    db.run('INSERT INTO messages (name, message, pfpuri, room_id, created_at) VALUES (?, ?, ?, ?, ?)', [name, sant, pfpuri, room_id, created_at], (err) => {
      if (err) {
        console.error(err.message);
        res.sendStatus(500);
      } else {
        io.emit(room_id, {name, message: sant, pfpuri, created_at});
        res.sendStatus(200);
      }
    });
  } else {
